...
Capability: AI Coding Agent Agentic Planning Tool Argument Checker 000466
...
_PLUGIN_NAME: str = 'AI Coding Agent Agentic Planning Tool Argument Checker 000466'
...
_PLUGIN_TAGS = ['coding', 'agents', 'agentic', 'planning', 'tools', 'arguments', 'safety', 'continuous_backlog', 'use_case_seeded', 'autonomous_factory', 'ai_progress']
...
def _run_core_logic(context: SkillContext, payload: Dict[str, Any], config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        plugin_name = 'AI Coding Agent Agentic Planning Tool Argument Checker 000466'
        ...
        use_cases = ['Apply this capability to the distinct use case: Coding Agent Agentic Planning.', 'Detect broad paths, destructive flags, and untrusted text.', 'Recommend bounded arguments before execution.', 'Separate safe read-only calls from risky mutations.', 'Show a compact progress state for this AI capability during baseline capability.', 'Return user-facing guidance that is useful, concise, and safe to act on.']
        payload_data = payload if isinstance(payload, dict) else {}
        payload_warnings = list(payload_data.get('_payload_warnings', [])) if isinstance(payload_data.get('_payload_warnings'), list) else []
        if not isinstance(payload, dict):
            payload_warnings.append('payload was not a dict; using empty payload')
