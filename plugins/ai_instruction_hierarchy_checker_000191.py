...
Capability: AI Instruction Hierarchy Checker 000191
Slug: ai_instruction_hierarchy_checker_000191
"""
...
_PLUGIN_NAME: str = 'AI Instruction Hierarchy Checker 000191'
_PLUGIN_SLUG: str = 'ai_instruction_hierarchy_checker_000191'
...
_PLUGIN_TAGS = ['healthcare', 'clinical', 'instructions', 'hierarchy', 'safety', 'continuous_backlog', 'use_case_seeded', 'autonomous_factory', 'ai_progress']
...
_PLUGIN_USE_CASES = ['Apply this capability to the distinct use case: Clinical Admin.', 'Find conflicts between instruction layers.', 'Flag unsafe or impossible requirements.', 'Recommend a clarified instruction set.', 'Show a compact progress state for this AI capability during baseline capability.', 'Return user-facing guidance that is useful, concise, and safe to act on.', 'Avoid duplicating existing AI capability behavior; identify what is unique about this capability.']
_PLUGIN_RESULT_SCHEMA_VERSION: str = "1.0.0"
_PLUGIN_MANIFEST = {'name': 'AI Instruction Hierarchy Checker 000191', 'slug': 'ai_instruction_hierarchy_checker_000191', 'goal': 'Check AI clinical administration and healthcare operations instructions for hierarchy conflicts, unsafe overrides, and impossible constraints.', 'category': 'ai_safety', 'tags': ['healthcare', 'clinical', 'instructions', 'hierarchy', 'safety', 'continuous_backlog', 'use_case_seeded', 'autonomous_factory', 'ai_progress'], 'version': '0.1.0', 'capability_type': 'scoring', 'intended_domain': 'AI clinical administration and healthcare operations instruction hierarchy and conflict safety', 'owner_id': None, 'use_cases': ['Apply this capability to the distinct use case: Clinical Admin.', 'Find conflicts between instruction layers.', 'Flag unsafe or impossible requirements.', 'Recommend a clarified instruction set.', 'Show a compact progress state for this AI capability during baseline capability.', 'Return user-facing guidance that is useful, concise, and safe to act on.', 'Avoid duplicating existing AI capability behavior; identify what is unique about this capability.'], 'schema_version': '1.0.0'}
...
def _run_core_logic(context: SkillContext, payload: Dict[str, Any], config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        plugin_name = 'AI Instruction Hierarchy Checker 000191'
        ...
        use_cases = ['Apply this capability to the distinct use case: Clinical Admin.', 'Find conflicts between instruction layers.', 'Flag unsafe or impossible requirements.', 'Recommend a clarified instruction set.', 'Show a compact progress state for this AI capability during baseline capability.', 'Return user-facing guidance that is useful, concise, and safe to act on.']
        payload_data = payload if isinstance(payload, dict) else {}
        payload_warnings = list(payload_data.get('_payload_warnings', [])) if isinstance(payload_data.get('_payload_warnings'), list) else []
        if not isinstance(payload, dict):
            payload_warnings.append('payload was not a dict; using empty payload')
