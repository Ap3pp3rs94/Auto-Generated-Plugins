...
# Title      : SalesForcePredictor
# File       : salesforcepredictor.py
# Description: Analyzes customer interaction data to identify at-risk accounts and provide actionable sales outreach recommendations, helping businesses proactively retain valuable customers and reduce churn.
# Goal       : Analyzes customer interaction data to identify at-risk accounts and provide actionable sales outreach recommendations, helping businesses proactively retain valuable customers and reduce churn.
# Category   : sales
# Tags       : customer_retention, predictive_analytics, sales_intelligence
# Version    : 0.1.0
...
_PLUGIN_NAME: str = 'SalesForcePredictor'
_PLUGIN_GOAL: str = 'Analyzes customer interaction data to identify at-risk accounts and provide actionable sales outreach recommendations, helping businesses proactively retain valuable customers and reduce churn.'
_PLUGIN_CATEGORY: str = 'sales'
_PLUGIN_TAGS = ['customer_retention', 'predictive_analytics', 'sales_intelligence']
_PLUGIN_VERSION: str = '0.1.0'
_PLUGIN_SLUG: str = 'salesforcepredictor'
...
def _run_core_logic(context: SkillContext, payload: Dict[str, Any]) -> Any:
    """Core plugin logic.
    ...
    def _run_core_logic(context: 'SkillContext', payload: Dict) -> None:
        if not isinstance(payload, dict) or 'customer_data' not in payload:
            # malformed input; provide a structured warning instead of erroring out
            result['summary'] = f"Cannot analyze customer data (missing or invalid format)"
            result['primary_insights'] = [
                {
                    'label': 'Data issue',
                    'description': 'Check the input format and re-run the analysis'
                }
            ]
            result['recommended_actions'] = ['Validate input data']
        else:
            customer_data = payload.get('customer_data', {})
            if not isinstance(customer_data, dict) or 'interactions' not in customer_data:
                # still malformed; provide a structured warning
                result['summary'] = f"Cannot analyze customer interactions (missing or invalid format)"
                result['primary_insights'] = [
                    {
                        'label': 'Data issue',
                        'description': 'Check the input format and re-run the analysis'
                ...
                if not isinstance(interactions, list):
                    # still malformed; provide a structured warning
                    result['summary'] = f"Cannot analyze customer interactions (invalid format)"
                    result['primary_insights'] = [
                        {
                            'label': 'Data issue',
                            'description': 'Check the input format and re-run the analysis'
                    ...
                    # successful execution; provide insights and recommendations
                    at_risk_accounts = []
                    for interaction in interactions:
                        if interaction.get('risk_level', 0) > 0.5:
                            at_risk_accounts.append(interaction.get('account_name'))
                    result['summary'] = f"Identified at-risk accounts: {', '.join(at_risk_accounts)}"
                    result['primary_insights'] = [
                        {
                            'label': 'At-risk accounts',
                            'description': 'Proactive outreach recommended for customer retention'
                        }
                    ]
                    result['recommended_actions'] = ['Send personalized emails or make targeted calls']
        # === LOGIC END ===
...
async def invoke(user_id: str, payload: Dict[str, Any], **kwargs: Any) -> Dict[str, Any]:
    ...
    try:
        maybe_output = _run_core_logic(ctx, payload)
