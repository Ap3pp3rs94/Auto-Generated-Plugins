...
# File       : customer_health_insights.py
# Description: Provides actionable customer health metrics and alert system for sales and account management teams to identify at-risk accounts and optimize retention efforts.
# Goal       : Provides actionable customer health metrics and alert system for sales and account management teams to identify at-risk accounts and optimize retention efforts.
# Category   : sales
# Tags       : customer_health, churn_prediction, retention_analysis
# Version    : 0.1.0
...
_PLUGIN_GOAL: str = 'Provides actionable customer health metrics and alert system for sales and account management teams to identify at-risk accounts and optimize retention efforts.'
_PLUGIN_CATEGORY: str = 'sales'
_PLUGIN_TAGS = ['customer_health', 'churn_prediction', 'retention_analysis']
_PLUGIN_VERSION: str = '0.1.0'
_PLUGIN_SLUG: str = 'customer_health_insights'
...
def _run_core_logic(context: SkillContext, payload: Dict[str, Any]) -> Any:
    ...
    result["summary"] = "Customer Health Insights: High-level overview of customer health metrics and recommendations for improvement."
    if 'customer_data' in payload:
        customer_data = payload['customer_data']
        if 'churn_risk_score' in customer_data:
            result["primary_insights"].append({"category": "Churn Risk", "description": f"Churn risk score: {customer_data['churn_risk_score']}."})
            if customer_data['churn_risk_score'] > 50:
                result["recommended_actions"].extend([
                    {"action": "Engage with the customer"},
                    {"action": "Offer a personalized upsell or cross-sell"}
                ])
    result["details"] = {
        "customer_health_metrics": ["Customer Health Score", "Churn Risk Score"],
