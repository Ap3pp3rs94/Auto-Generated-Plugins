...
Plugin: AI Plugin Logic Blueprint Designer Phase 9
Slug: ai_plugin_logic_blueprint_designer_phase_9
"""
...
_PLUGIN_NAME: str = 'AI Plugin Logic Blueprint Designer Phase 9'
_PLUGIN_SLUG: str = 'ai_plugin_logic_blueprint_designer_phase_9'
_PLUGIN_CATEGORY: str = 'ai_plugin_factory'
_PLUGIN_VERSION: str = '0.1.0'
_PLUGIN_GOAL: str = 'Extend AI Plugin Logic Blueprint Designer with phase 9 behavior: preserve the original AI capability, add stronger edge-case handling, expose clearer user-facing progress signals, and produce more actionable next steps.'
_PLUGIN_TAGS = ['ai', 'plugins', 'logic', 'factory', 'autonomous_factory', 'ai_progress', 'phase_9']
_PLUGIN_OWNER_ID = 'francis-factory'
_PLUGIN_CAPABILITY_TYPE = 'system_automation'
_PLUGIN_INTENDED_DOMAIN = 'AI plugin deterministic logic planning and implementation design'
_PLUGIN_USE_CASES = ['Translate a PluginSpec into deterministic analysis steps.', 'Define payload fields, scoring signals, and output constructors.', 'Prevent generic template bodies by naming capability-specific algorithms.', 'Show a compact progress state for this AI capability during advanced refinement pass 9.', 'Return user-facing guidance that is useful, concise, and safe to act on.', 'Avoid duplicating existing AI plugin behavior; identify what is unique about this capability.']
_PLUGIN_RESULT_SCHEMA_VERSION: str = '1.0.0'

# Lightweight manifest for discovery/registry.
_PLUGIN_MANIFEST = {'name': 'AI Plugin Logic Blueprint Designer Phase 9', 'slug': 'ai_plugin_logic_blueprint_designer_phase_9', 'goal': 'Extend AI Plugin Logic Blueprint Designer with phase 9 behavior: preserve the original AI capability, add stronger edge-case handling, expose clearer user-facing progress signals, and produce more actionable next steps.', 'category': 'ai_plugin_factory', 'tags': ['ai', 'plugins', 'logic', 'factory', 'autonomous_factory', 'ai_progress', 'phase_9'], 'version': '0.1.0', 'capability_type': 'system_automation', 'intended_domain': 'AI plugin deterministic logic planning and implementation design', 'owner_id': 'francis-factory', 'use_cases': ['Translate a PluginSpec into deterministic analysis steps.', 'Define payload fields, scoring signals, and output constructors.', 'Prevent generic template bodies by naming capability-specific algorithms.', 'Show a compact progress state for this AI capability during advanced refinement pass 9.', 'Return user-facing guidance that is useful, concise, and safe to act on.', 'Avoid duplicating existing AI plugin behavior; identify what is unique about this capability.'], 'primary_use_case': 'Translate a PluginSpec into deterministic analysis steps.', 'problem_statement': 'Francis needs a focused AI plugin for AI plugin deterministic logic planning and implementation design. The plugin must turn messy user notes, model outputs, traces, or workflow state into structured, actionable AI assistance. It should improve autonomous progress by making the next step obvious, reducing duplicated work, and exposing risks before they become failures. The plugin must stay deterministic, avoid hidden external side effects, and make its reasoning inspectable through concise details and scores.', 'primary_inputs': 'A payload dict that may contain task, objective, prompt, conversation, artifacts, candidate_outputs, traces, rubric, constraints, user_level, current_plan, completed_steps, blocked_steps, source_notes, and optional customer_config.', 'primary_outputs': 'A normalized result dict with summary, primary_insights, recommended_actions, scores, details, progress_state, user_experience, fun_mode, and machine-readable diagnostics.', 'constraints': 'Do not execute tools, browse, mutate files, call external APIs, or claim facts not present in the payload. Prefer deterministic analysis. If data is missing, say what is missing and return a useful fallback. Keep recommendations concrete. Avoid creating a capability that duplicates another AI roadmap plugin.', 'example_use_cases': ['A user asks Francis to improve an AI workflow related to AI plugin deterministic logic planning and implementation design.', 'A long-running autonomous run needs a checkpoint summary and the next best action.', 'An operator wants a scorecard that separates hard failures from polish improvements.', 'A beginner wants the result explained in plain language without losing technical accuracy.', 'A power user wants structured JSON they can pipe into another agent or dashboard.'], 'io_contract': "Input: payload is a dict. Important optional keys include 'task', 'objective', 'prompt', 'conversation', 'messages', 'candidate_outputs', 'trace', 'rubric', 'constraints', 'progress', 'current_plan', 'completed_steps', 'blocked_steps', 'user_level', and 'customer_config'. Output: return a dict containing summary: str, primary_insights: list, recommended_actions: list, scores: dict with confidence and usefulness, details: dict, progress_state: dict with current_stage/next_step/blockers, user_experience: dict with plain_language_takeaway and interaction_suggestions, and fun_mode: dict with optional challenge_label, score_badge, and celebratory_microcopy. Fun fields must never replace the serious recommendation.", 'schema_version': '1.0.0', 'family_key': 'ai_plugin_factory::ai plugin logic blueprint designer phase', 'is_generic_name': False}
...
def _run_core_logic(context: SkillContext, payload: Dict[str, Any], config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        plugin_name = 'AI Plugin Logic Blueprint Designer Phase 9'
        goal = 'Extend AI Plugin Logic Blueprint Designer with phase 9 behavior: preserve the original AI capability, add stronger edge-case handling, expose clearer user-facing progress signals, and produce more actionable next steps.'
        domain = 'AI plugin deterministic logic planning and implementation design'
        capability_type = 'system_automation'
        logic_profile_id = 'plugin_logic_blueprint_designer_profile'
        generation_note = 'capability profile registry override'
        use_cases = ['Translate a PluginSpec into deterministic analysis steps.', 'Define payload fields, scoring signals, and output constructors.', 'Prevent generic template bodies by naming capability-specific algorithms.', 'Show a compact progress state for this AI capability during advanced refinement pass 9.', 'Return user-facing guidance that is useful, concise, and safe to act on.', 'Avoid duplicating existing AI plugin behavior; identify what is unique about this capability.']
