...
Plugin: AI User Clarification Question Builder
...
_PLUGIN_NAME: str = 'AI User Clarification Question Builder'
_PLUGIN_SLUG: str = 'ai_user_clarification_question_builder'
_PLUGIN_CATEGORY: str = 'ai_prompting'
_PLUGIN_VERSION: str = '0.1.0'
_PLUGIN_GOAL: str = 'Generate the smallest useful set of clarification questions when an AI task is under-specified.'
_PLUGIN_TAGS = ['ai', 'prompting', 'clarification', 'requirements', 'autonomous_factory', 'ai_progress', 'phase_1']
_PLUGIN_OWNER_ID = 'francis-factory'
_PLUGIN_CAPABILITY_TYPE = 'enrichment'
_PLUGIN_INTENDED_DOMAIN = 'AI clarification strategy and missing input collection'
_PLUGIN_USE_CASES = ['Ask only questions that unblock execution.', 'Avoid unnecessary clarification when assumptions are safe.', 'Group questions by objective, constraints, data, and output format.', 'Show a compact progress state for this AI capability during baseline capability.', 'Return user-facing guidance that is useful, concise, and safe to act on.', 'Avoid duplicating existing AI capability behavior; identify what is unique about this capability.']
_PLUGIN_RESULT_SCHEMA_VERSION: str = '1.0.0'

# Lightweight manifest for discovery/registry.
_PLUGIN_MANIFEST = {'name': 'AI User Clarification Question Builder', 'slug': 'ai_user_clarification_question_builder', 'goal': 'Generate the smallest useful set of clarification questions when an AI task is under-specified.', 'category': 'ai_prompting', 'tags': ['ai', 'prompting', 'clarification', 'requirements', 'autonomous_factory', 'ai_progress', 'phase_1'], 'version': '0.1.0', 'capability_type': 'enrichment', 'intended_domain': 'AI clarification strategy and missing input collection', 'owner_id': 'francis-factory', 'use_cases': ['Ask only questions that unblock execution.', 'Avoid unnecessary clarification when assumptions are safe.', 'Group questions by objective, constraints, data, and output format.', 'Show a compact progress state for this AI capability during baseline capability.', 'Return user-facing guidance that is useful, concise, and safe to act on.', 'Avoid duplicating existing AI capability behavior; identify what is unique about this capability.'], 'primary_use_case': 'Ask only questions that unblock execution.', 'problem_statement': 'Francis needs a focused AI capability for AI clarification strategy and missing input collection. The generated module must turn messy user notes, model outputs, traces, or workflow state into structured, actionable AI assistance. It should improve autonomous progress by making the next step obvious, reducing duplicated work, and exposing risks before they become failures. The capability must stay deterministic, avoid hidden external side effects, and make its reasoning inspectable through concise details and scores.', 'primary_inputs': 'A payload dict that may contain task, objective, prompt, conversation, artifacts, candidate_outputs, traces, rubric, constraints, user_level, current_plan, completed_steps, blocked_steps, source_notes, and optional customer_config.', 'primary_outputs': 'A normalized result dict with summary, primary_insights, recommended_actions, scores, details, progress_state, user_experience, fun_mode, and machine-readable diagnostics.', 'constraints': 'Do not execute tools, browse, mutate files, call external APIs, or claim facts not present in the payload. Prefer deterministic analysis. If data is missing, say what is missing and return a useful fallback. Keep recommendations concrete. Avoid creating a capability that duplicates another AI roadmap capability.', 'example_use_cases': ['A user asks Francis to improve an AI workflow related to AI clarification strategy and missing input collection.', 'A long-running autonomous run needs a checkpoint summary and the next best action.', 'An operator wants a scorecard that separates hard failures from polish improvements.', 'A beginner wants the result explained in plain language without losing technical accuracy.', 'A power user wants structured JSON they can pipe into another agent or dashboard.'], 'io_contract': "Input: payload is a dict. Important optional keys include 'task', 'objective', 'prompt', 'conversation', 'messages', 'candidate_outputs', 'trace', 'rubric', 'constraints', 'progress', 'current_plan', 'completed_steps', 'blocked_steps', 'user_level', and 'customer_config'. Output: return a dict containing summary: str, primary_insights: list, recommended_actions: list, scores: dict with confidence and usefulness, details: dict, progress_state: dict with current_stage/next_step/blockers, user_experience: dict with plain_language_takeaway and interaction_suggestions, and fun_mode: dict with optional challenge_label, score_badge, and celebratory_microcopy. Fun fields must never replace the serious recommendation.", 'schema_version': '1.0.0', 'family_key': 'ai_prompting::ai user clarification question builder', 'is_generic_name': False}
...
def _run_core_logic(context: SkillContext, payload: Dict[str, Any], config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        plugin_name = 'AI User Clarification Question Builder'
        goal = 'Generate the smallest useful set of clarification questions when an AI task is under-specified.'
        domain = 'AI clarification strategy and missing input collection'
        capability_type = 'enrichment'
        logic_profile_id = 'user_clarification_question_builder_profile'
