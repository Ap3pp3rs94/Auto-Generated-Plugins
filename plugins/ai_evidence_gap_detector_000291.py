...
Capability: AI Evidence Gap Detector 000291
Slug: ai_evidence_gap_detector_000291
"""
...
_PLUGIN_NAME: str = 'AI Evidence Gap Detector 000291'
_PLUGIN_SLUG: str = 'ai_evidence_gap_detector_000291'
...
_PLUGIN_TAGS = ['data', 'analytics', 'retrieval', 'evidence', 'grounding', 'continuous_backlog', 'use_case_seeded', 'autonomous_factory', 'ai_progress']
...
_PLUGIN_USE_CASES = ['Apply this capability to the distinct use case: Data Analysis.', 'List unsupported claims in Data Analysis work.', 'Rank evidence gaps by risk and retrieval priority.', 'Suggest focused retrieval questions for each missing source.', 'Show a compact progress state for this AI capability during baseline capability.', 'Return user-facing guidance that is useful, concise, and safe to act on.', 'Avoid duplicating existing AI capability behavior; identify what is unique about this capability.']
_PLUGIN_RESULT_SCHEMA_VERSION: str = "1.0.0"
_PLUGIN_MANIFEST = {'name': 'AI Evidence Gap Detector 000291', 'slug': 'ai_evidence_gap_detector_000291', 'goal': 'Detect missing evidence that blocks trustworthy answers in AI data analysis and analytics workflows.', 'category': 'ai_retrieval', 'tags': ['data', 'analytics', 'retrieval', 'evidence', 'grounding', 'continuous_backlog', 'use_case_seeded', 'autonomous_factory', 'ai_progress'], 'version': '0.1.0', 'capability_type': 'research_synthesizer', 'intended_domain': 'AI data analysis and analytics workflows evidence gaps and retrieval readiness', 'owner_id': None, 'use_cases': ['Apply this capability to the distinct use case: Data Analysis.', 'List unsupported claims in Data Analysis work.', 'Rank evidence gaps by risk and retrieval priority.', 'Suggest focused retrieval questions for each missing source.', 'Show a compact progress state for this AI capability during baseline capability.', 'Return user-facing guidance that is useful, concise, and safe to act on.', 'Avoid duplicating existing AI capability behavior; identify what is unique about this capability.'], 'schema_version': '1.0.0'}
...
def _run_core_logic(context: SkillContext, payload: Dict[str, Any], config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        plugin_name = 'AI Evidence Gap Detector 000291'
        ...
        use_cases = ['Apply this capability to the distinct use case: Data Analysis.', 'List unsupported claims in Data Analysis work.', 'Rank evidence gaps by risk and retrieval priority.', 'Suggest focused retrieval questions for each missing source.', 'Show a compact progress state for this AI capability during baseline capability.', 'Return user-facing guidance that is useful, concise, and safe to act on.']
        payload_data = payload if isinstance(payload, dict) else {}
        payload_warnings = list(payload_data.get('_payload_warnings', [])) if isinstance(payload_data.get('_payload_warnings'), list) else []
        if not isinstance(payload, dict):
            payload_warnings.append('payload was not a dict; using empty payload')
