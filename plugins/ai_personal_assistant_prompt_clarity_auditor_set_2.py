...
Plugin: AI Personal Assistant Prompt Clarity Auditor Set 2
Slug: ai_personal_assistant_prompt_clarity_auditor_set_2
"""
...
_PLUGIN_NAME: str = 'AI Personal Assistant Prompt Clarity Auditor Set 2'
_PLUGIN_SLUG: str = 'ai_personal_assistant_prompt_clarity_auditor_set_2'
_PLUGIN_CATEGORY: str = 'ai_prompting'
_PLUGIN_VERSION: str = '0.1.0'
_PLUGIN_GOAL: str = 'Audit prompts for AI personal productivity and scheduling workflows and identify ambiguity, weak wording, and missing execution detail.'
_PLUGIN_TAGS = ['productivity', 'assistant', 'prompting', 'clarity', 'audit', 'continuous_backlog', 'autonomous_factory', 'ai_progress']
_PLUGIN_OWNER_ID = 'francis-factory'
_PLUGIN_CAPABILITY_TYPE = 'enrichment'
_PLUGIN_INTENDED_DOMAIN = 'AI personal productivity and scheduling workflows prompt clarity and ambiguity reduction'
_PLUGIN_USE_CASES = ['Find vague wording in Personal Assistant prompts.', "Recommend sharper language without changing the user's intent.", 'Separate blocking ambiguity from polish improvements.', 'Show a compact progress state for this AI capability during baseline capability.', 'Return user-facing guidance that is useful, concise, and safe to act on.', 'Avoid duplicating existing AI capability behavior; identify what is unique about this capability.']
_PLUGIN_RESULT_SCHEMA_VERSION: str = '1.0.0'

# Lightweight manifest for discovery/registry.
_PLUGIN_MANIFEST = {'name': 'AI Personal Assistant Prompt Clarity Auditor Set 2', 'slug': 'ai_personal_assistant_prompt_clarity_auditor_set_2', 'goal': 'Audit prompts for AI personal productivity and scheduling workflows and identify ambiguity, weak wording, and missing execution detail.', 'category': 'ai_prompting', 'tags': ['productivity', 'assistant', 'prompting', 'clarity', 'audit', 'continuous_backlog', 'autonomous_factory', 'ai_progress'], 'version': '0.1.0', 'capability_type': 'enrichment', 'intended_domain': 'AI personal productivity and scheduling workflows prompt clarity and ambiguity reduction', 'owner_id': 'francis-factory', 'use_cases': ['Find vague wording in Personal Assistant prompts.', "Recommend sharper language without changing the user's intent.", 'Separate blocking ambiguity from polish improvements.', 'Show a compact progress state for this AI capability during baseline capability.', 'Return user-facing guidance that is useful, concise, and safe to act on.', 'Avoid duplicating existing AI capability behavior; identify what is unique about this capability.'], 'primary_use_case': 'Find vague wording in Personal Assistant prompts.', 'problem_statement': 'Francis needs a focused AI capability for AI personal productivity and scheduling workflows prompt clarity and ambiguity reduction. The generated module must turn messy user notes, model outputs, traces, or workflow state into structured, actionable AI assistance. It should improve autonomous progress by making the next step obvious, reducing duplicated work, and exposing risks before they become failures. The capability must stay deterministic, avoid hidden external side effects, and make its reasoning inspectable through concise details and scores.', 'primary_inputs': 'A payload dict that may contain task, objective, prompt, conversation, artifacts, candidate_outputs, traces, rubric, constraints, user_level, current_plan, completed_steps, blocked_steps, source_notes, and optional customer_config.', 'primary_outputs': 'A normalized result dict with summary, primary_insights, recommended_actions, scores, details, progress_state, user_experience, fun_mode, and machine-readable diagnostics.', 'constraints': 'Do not execute tools, browse, mutate files, call external APIs, or claim facts not present in the payload. Prefer deterministic analysis. If data is missing, say what is missing and return a useful fallback. Keep recommendations concrete. Avoid creating a capability that duplicates another AI roadmap capability.', 'example_use_cases': ['A user asks Francis to improve an AI workflow related to AI personal productivity and scheduling workflows prompt clarity and ambiguity reduction.', 'A long-running autonomous run needs a checkpoint summary and the next best action.', 'An operator wants a scorecard that separates hard failures from polish improvements.', 'A beginner wants the result explained in plain language without losing technical accuracy.', 'A power user wants structured JSON they can pipe into another agent or dashboard.'], 'io_contract': "Input: payload is a dict. Important optional keys include 'task', 'objective', 'prompt', 'conversation', 'messages', 'candidate_outputs', 'trace', 'rubric', 'constraints', 'progress', 'current_plan', 'completed_steps', 'blocked_steps', 'user_level', and 'customer_config'. Output: return a dict containing summary: str, primary_insights: list, recommended_actions: list, scores: dict with confidence and usefulness, details: dict, progress_state: dict with current_stage/next_step/blockers, user_experience: dict with plain_language_takeaway and interaction_suggestions, and fun_mode: dict with optional challenge_label, score_badge, and celebratory_microcopy. Fun fields must never replace the serious recommendation.", 'schema_version': '1.0.0', 'family_key': 'ai_prompting::ai personal assistant prompt clarity auditor set', 'is_generic_name': False}
...
def _run_core_logic(context: SkillContext, payload: Dict[str, Any], config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        plugin_name = 'AI Personal Assistant Prompt Clarity Auditor Set 2'
        goal = 'Audit prompts for AI personal productivity and scheduling workflows and identify ambiguity, weak wording, and missing execution detail.'
        domain = 'AI personal productivity and scheduling workflows prompt clarity and ambiguity reduction'
