...
Capability: AI Capability Factory Agent Handoff Checker 000439
...
_PLUGIN_NAME: str = 'AI Capability Factory Agent Handoff Checker 000439'
...
_PLUGIN_TAGS = ['capabilities', 'factory', 'agents', 'handoff', 'coordination', 'continuous_backlog', 'use_case_seeded', 'autonomous_factory', 'ai_progress']
...
_PLUGIN_USE_CASES = ['Apply this capability to the distinct use case: Capability Factory.', 'Validate handoff packets before another agent starts.', 'Detect duplicated ownership and missing changed-file context.', 'Recommend handoff fixes and integration checkpoints.', 'Show a compact progress state for this AI capability during baseline capability.', 'Return user-facing guidance that is useful, concise, and safe to act on.', 'Avoid duplicating existing AI capability behavior; identify what is unique about this capability.']
_PLUGIN_RESULT_SCHEMA_VERSION: str = "1.0.0"
_PLUGIN_MANIFEST = {'name': 'AI Capability Factory Agent Handoff Checker 000439', 'slug': 'ai_plugin_factory_agent_handoff_checker', 'goal': 'Check AI capability factory and generated module operations agent handoffs for missing context, ownership overlap, and unsafe next steps.', 'category': 'ai_agents', 'tags': ['capabilities', 'factory', 'agents', 'handoff', 'coordination', 'continuous_backlog', 'use_case_seeded', 'autonomous_factory', 'ai_progress'], 'version': '0.1.0', 'capability_type': 'system_automation', 'intended_domain': 'AI capability factory and generated module operations agent handoff quality and coordination', 'owner_id': 'francis-factory', 'use_cases': ['Apply this capability to the distinct use case: Capability Factory.', 'Validate handoff packets before another agent starts.', 'Detect duplicated ownership and missing changed-file context.', 'Recommend handoff fixes and integration checkpoints.', 'Show a compact progress state for this AI capability during baseline capability.', 'Return user-facing guidance that is useful, concise, and safe to act on.', 'Avoid duplicating existing AI capability behavior; identify what is unique about this capability.'], 'schema_version': '1.0.0'}
...
def _run_core_logic(context: SkillContext, payload: Dict[str, Any], config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        plugin_name = 'AI Capability Factory Agent Handoff Checker 000439'
        ...
        use_cases = ['Apply this capability to the distinct use case: Capability Factory.', 'Validate handoff packets before another agent starts.', 'Detect duplicated ownership and missing changed-file context.', 'Recommend handoff fixes and integration checkpoints.', 'Show a compact progress state for this AI capability during baseline capability.', 'Return user-facing guidance that is useful, concise, and safe to act on.']
        payload_data = payload if isinstance(payload, dict) else {}
        payload_warnings = list(payload_data.get('_payload_warnings', [])) if isinstance(payload_data.get('_payload_warnings'), list) else []
        if not isinstance(payload, dict):
            payload_warnings.append('payload was not a dict; using empty payload')
