...
Capability: AI Data Analysis Response Action Planner 000300
...
_PLUGIN_NAME: str = 'AI Data Analysis Response Action Planner 000300'
...
_PLUGIN_TAGS = ['data', 'analytics', 'evaluation', 'actions', 'planning', 'continuous_backlog', 'use_case_seeded', 'autonomous_factory', 'ai_progress']
...
_PLUGIN_USE_CASES = ['Apply this capability to the distinct use case: Data Analysis.', 'Find vague advice that needs executable next steps.', 'Create action items with checks and risk notes.', 'Score whether the response is ready to act on.', 'Show a compact progress state for this AI capability during baseline capability.', 'Return user-facing guidance that is useful, concise, and safe to act on.', 'Avoid duplicating existing AI capability behavior; identify what is unique about this capability.']
_PLUGIN_RESULT_SCHEMA_VERSION: str = "1.0.0"
_PLUGIN_MANIFEST = {'name': 'AI Data Analysis Response Action Planner 000300', 'slug': 'ai_data_analysis_response_action_planner', 'goal': 'Convert AI data analysis and analytics workflows AI responses into concrete next actions with owners, checks, and risks.', 'category': 'ai_evaluation', 'tags': ['data', 'analytics', 'evaluation', 'actions', 'planning', 'continuous_backlog', 'use_case_seeded', 'autonomous_factory', 'ai_progress'], 'version': '0.1.0', 'capability_type': 'data_insight', 'intended_domain': 'AI data analysis and analytics workflows response actionability and next-step planning', 'owner_id': 'francis-factory', 'use_cases': ['Apply this capability to the distinct use case: Data Analysis.', 'Find vague advice that needs executable next steps.', 'Create action items with checks and risk notes.', 'Score whether the response is ready to act on.', 'Show a compact progress state for this AI capability during baseline capability.', 'Return user-facing guidance that is useful, concise, and safe to act on.', 'Avoid duplicating existing AI capability behavior; identify what is unique about this capability.'], 'schema_version': '1.0.0'}
...
def _run_core_logic(context: SkillContext, payload: Dict[str, Any], config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        plugin_name = 'AI Data Analysis Response Action Planner 000300'
        ...
        use_cases = ['Apply this capability to the distinct use case: Data Analysis.', 'Find vague advice that needs executable next steps.', 'Create action items with checks and risk notes.', 'Score whether the response is ready to act on.', 'Show a compact progress state for this AI capability during baseline capability.', 'Return user-facing guidance that is useful, concise, and safe to act on.']
        payload_data = payload if isinstance(payload, dict) else {}
        payload_warnings = list(payload_data.get('_payload_warnings', [])) if isinstance(payload_data.get('_payload_warnings'), list) else []
        if not isinstance(payload, dict):
            payload_warnings.append('payload was not a dict; using empty payload')
