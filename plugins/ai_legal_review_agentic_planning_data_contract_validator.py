...
Capability: AI Legal Review Agentic Planning Data Contract Validator 000600
...
_PLUGIN_TAGS = ['legal', 'compliance', 'agentic', 'planning', 'schema', 'contracts', 'validation', 'continuous_backlog', 'use_case_seeded', 'autonomous_factory', 'ai_progress']
...
_PLUGIN_USE_CASES = ['Apply this capability to the distinct use case: Legal Review Agentic Planning.', 'Extract expected payload and result fields.', 'Flag schema ambiguity before implementation.', 'Recommend validation checks for downstream consumers.', 'Show a compact progress state for this AI capability during baseline capability.', 'Return user-facing guidance that is useful, concise, and safe to act on.', 'Avoid duplicating existing AI capability behavior; identify what is unique about this capability.']
_PLUGIN_RESULT_SCHEMA_VERSION: str = "1.0.0"
_PLUGIN_MANIFEST = {'name': 'AI Legal Review Agentic Planning Data Contract Validator 000600', 'slug': 'ai_legal_review_agentic_planning_data_contract_validator', 'goal': 'Validate expected input and output contracts for AI legal review and compliance workflows for agentic planning AI capabilities.', 'category': 'ai_evaluation', 'tags': ['legal', 'compliance', 'agentic', 'planning', 'schema', 'contracts', 'validation', 'continuous_backlog', 'use_case_seeded', 'autonomous_factory', 'ai_progress'], 'version': '0.1.0', 'capability_type': 'enrichment', 'intended_domain': 'AI legal review and compliance workflows for agentic planning data contract validation and schema readiness', 'owner_id': 'francis-factory', 'use_cases': ['Apply this capability to the distinct use case: Legal Review Agentic Planning.', 'Extract expected payload and result fields.', 'Flag schema ambiguity before implementation.', 'Recommend validation checks for downstream consumers.', 'Show a compact progress state for this AI capability during baseline capability.', 'Return user-facing guidance that is useful, concise, and safe to act on.', 'Avoid duplicating existing AI capability behavior; identify what is unique about this capability.'], 'schema_version': '1.0.0'}
...
def _run_core_logic(context: SkillContext, payload: Dict[str, Any], config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        use_cases = ['Apply this capability to the distinct use case: Legal Review Agentic Planning.', 'Extract expected payload and result fields.', 'Flag schema ambiguity before implementation.', 'Recommend validation checks for downstream consumers.', 'Show a compact progress state for this AI capability during baseline capability.', 'Return user-facing guidance that is useful, concise, and safe to act on.']
        payload_data = payload if isinstance(payload, dict) else {}
        payload_warnings = list(payload_data.get('_payload_warnings', [])) if isinstance(payload_data.get('_payload_warnings'), list) else []
        if not isinstance(payload, dict):
            payload_warnings.append('payload was not a dict; using empty payload')
