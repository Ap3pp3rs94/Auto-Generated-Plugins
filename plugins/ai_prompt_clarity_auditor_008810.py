...
Capability: AI Prompt Clarity Auditor 008810
Slug: ai_prompt_clarity_auditor_008810
"""
...
_PLUGIN_NAME: str = 'AI Prompt Clarity Auditor 008810'
_PLUGIN_SLUG: str = 'ai_prompt_clarity_auditor_008810'
_PLUGIN_CATEGORY: str = 'ai_prompting'
_PLUGIN_VERSION: str = '0.1.0'
_PLUGIN_GOAL: str = 'Audit prompts for AI content strategy and editorial workflows for AI tool use for detecting signals and identify ambiguity, weak wording, and missing execution detail.'
_PLUGIN_TAGS = ['content', 'strategy', 'tools', 'tool-use', 'detect', 'prompting', 'clarity', 'audit', 'continuous_backlog', 'use_case_seeded', 'autonomous_factory', 'ai_progress']
_PLUGIN_OWNER_ID = 'francis-factory'
_PLUGIN_CAPABILITY_TYPE = 'enrichment'
_PLUGIN_INTENDED_DOMAIN = 'AI content strategy and editorial workflows for AI tool use for detecting signals prompt clarity and ambiguity reduction'
_PLUGIN_USE_CASES = ['Apply this capability to the distinct use case: Content Strategy Tool Use Detection.', 'Find vague wording in Content Strategy Tool Use Detection prompts.', "Recommend sharper language without changing the user's intent.", 'Separate blocking ambiguity from polish improvements.', 'Show a compact progress state for this AI capability during baseline capability.', 'Return user-facing guidance that is useful, concise, and safe to act on.', 'Avoid duplicating existing AI capability behavior; identify what is unique about this capability.']
_PLUGIN_RESULT_SCHEMA_VERSION: str = "1.0.0"
_PLUGIN_MANIFEST = {'name': 'AI Prompt Clarity Auditor 008810', 'slug': 'ai_prompt_clarity_auditor_008810', 'goal': 'Audit prompts for AI content strategy and editorial workflows for AI tool use for detecting signals and identify ambiguity, weak wording, and missing execution detail.', 'category': 'ai_prompting', 'tags': ['content', 'strategy', 'tools', 'tool-use', 'detect', 'prompting', 'clarity', 'audit', 'continuous_backlog', 'use_case_seeded', 'autonomous_factory', 'ai_progress'], 'version': '0.1.0', 'capability_type': 'enrichment', 'intended_domain': 'AI content strategy and editorial workflows for AI tool use for detecting signals prompt clarity and ambiguity reduction', 'owner_id': 'francis-factory', 'use_cases': ['Apply this capability to the distinct use case: Content Strategy Tool Use Detection.', 'Find vague wording in Content Strategy Tool Use Detection prompts.', "Recommend sharper language without changing the user's intent.", 'Separate blocking ambiguity from polish improvements.', 'Show a compact progress state for this AI capability during baseline capability.', 'Return user-facing guidance that is useful, concise, and safe to act on.', 'Avoid duplicating existing AI capability behavior; identify what is unique about this capability.'], 'schema_version': '1.0.0'}
...
def _run_core_logic(context: SkillContext, payload: Dict[str, Any], config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        plugin_name = 'AI Prompt Clarity Auditor 008810'
        goal = 'Audit prompts for AI content strategy and editorial workflows for AI tool use for detecting signals and identify ambiguity, weak wording, and missing execution detail.'
        domain = 'AI content strategy and editorial workflows for AI tool use for detecting signals prompt clarity and ambiguity reduction'
        ...
        use_cases = ['Apply this capability to the distinct use case: Content Strategy Tool Use Detection.', 'Find vague wording in Content Strategy Tool Use Detection prompts.', "Recommend sharper language without changing the user's intent.", 'Separate blocking ambiguity from polish improvements.', 'Show a compact progress state for this AI capability during baseline capability.', 'Return user-facing guidance that is useful, concise, and safe to act on.']
        payload_data = payload if isinstance(payload, dict) else {}
        payload_warnings = list(payload_data.get('_payload_warnings', [])) if isinstance(payload_data.get('_payload_warnings'), list) else []
        if not isinstance(payload, dict):
            payload_warnings.append('payload was not a dict; using empty payload')
