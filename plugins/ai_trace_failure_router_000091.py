...
Capability: AI Trace Failure Router 000091
Slug: ai_trace_failure_router_000091
"""
...
_PLUGIN_NAME: str = 'AI Trace Failure Router 000091'
_PLUGIN_SLUG: str = 'ai_trace_failure_router_000091'
...
_PLUGIN_TAGS = ['coding', 'agents', 'traces', 'failures', 'debugging', 'continuous_backlog', 'use_case_seeded', 'autonomous_factory', 'ai_progress']
...
_PLUGIN_USE_CASES = ['Apply this capability to the distinct use case: Coding Agent.', 'Classify failures by prompt, context, tool, retrieval, or validation cause.', 'Recommend the smallest retry strategy.', 'Preserve useful progress from failed runs.', 'Show a compact progress state for this AI capability during baseline capability.', 'Return user-facing guidance that is useful, concise, and safe to act on.', 'Avoid duplicating existing AI capability behavior; identify what is unique about this capability.']
_PLUGIN_RESULT_SCHEMA_VERSION: str = "1.0.0"
_PLUGIN_MANIFEST = {'name': 'AI Trace Failure Router 000091', 'slug': 'ai_trace_failure_router_000091', 'goal': 'Route AI coding agents and repository work trace failures to the most likely repair path.', 'category': 'ai_agents', 'tags': ['coding', 'agents', 'traces', 'failures', 'debugging', 'continuous_backlog', 'use_case_seeded', 'autonomous_factory', 'ai_progress'], 'version': '0.1.0', 'capability_type': 'data_insight', 'intended_domain': 'AI coding agents and repository work trace failure classification and repair routing', 'owner_id': None, 'use_cases': ['Apply this capability to the distinct use case: Coding Agent.', 'Classify failures by prompt, context, tool, retrieval, or validation cause.', 'Recommend the smallest retry strategy.', 'Preserve useful progress from failed runs.', 'Show a compact progress state for this AI capability during baseline capability.', 'Return user-facing guidance that is useful, concise, and safe to act on.', 'Avoid duplicating existing AI capability behavior; identify what is unique about this capability.'], 'schema_version': '1.0.0'}
...
def _run_core_logic(context: SkillContext, payload: Dict[str, Any], config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        plugin_name = 'AI Trace Failure Router 000091'
        ...
        use_cases = ['Apply this capability to the distinct use case: Coding Agent.', 'Classify failures by prompt, context, tool, retrieval, or validation cause.', 'Recommend the smallest retry strategy.', 'Preserve useful progress from failed runs.', 'Show a compact progress state for this AI capability during baseline capability.', 'Return user-facing guidance that is useful, concise, and safe to act on.']
        payload_data = payload if isinstance(payload, dict) else {}
        payload_warnings = list(payload_data.get('_payload_warnings', [])) if isinstance(payload_data.get('_payload_warnings'), list) else []
        if not isinstance(payload, dict):
            payload_warnings.append('payload was not a dict; using empty payload')
