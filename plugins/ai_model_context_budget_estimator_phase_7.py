...
Plugin: AI Model Context Budget Estimator Phase 7
Slug: ai_model_context_budget_estimator_phase_7
"""
...
_PLUGIN_NAME: str = 'AI Model Context Budget Estimator Phase 7'
_PLUGIN_SLUG: str = 'ai_model_context_budget_estimator_phase_7'
_PLUGIN_CATEGORY: str = 'ai_memory'
_PLUGIN_VERSION: str = '0.1.0'
_PLUGIN_GOAL: str = 'Extend AI Model Context Budget Estimator with phase 7 behavior: preserve the original AI capability, add stronger edge-case handling, expose clearer user-facing progress signals, and produce more actionable next steps.'
_PLUGIN_TAGS = ['ai', 'context', 'tokens', 'budgeting', 'autonomous_factory', 'ai_progress', 'phase_7']
_PLUGIN_OWNER_ID = 'francis-factory'
_PLUGIN_CAPABILITY_TYPE = 'data_insight'
_PLUGIN_INTENDED_DOMAIN = 'AI context budgeting and model-call preparation'
_PLUGIN_USE_CASES = ['Estimate whether a payload will fit a target context window.', 'Flag high-value context that should survive compression.', 'Recommend token-saving cuts without losing requirements.', 'Show a compact progress state for this AI capability during advanced refinement pass 7.', 'Return user-facing guidance that is useful, concise, and safe to act on.', 'Avoid duplicating existing AI capability behavior; identify what is unique about this capability.']
_PLUGIN_RESULT_SCHEMA_VERSION: str = '1.0.0'

# Lightweight manifest for discovery/registry.
_PLUGIN_MANIFEST = {'name': 'AI Model Context Budget Estimator Phase 7', 'slug': 'ai_model_context_budget_estimator_phase_7', 'goal': 'Extend AI Model Context Budget Estimator with phase 7 behavior: preserve the original AI capability, add stronger edge-case handling, expose clearer user-facing progress signals, and produce more actionable next steps.', 'category': 'ai_memory', 'tags': ['ai', 'context', 'tokens', 'budgeting', 'autonomous_factory', 'ai_progress', 'phase_7'], 'version': '0.1.0', 'capability_type': 'data_insight', 'intended_domain': 'AI context budgeting and model-call preparation', 'owner_id': 'francis-factory', 'use_cases': ['Estimate whether a payload will fit a target context window.', 'Flag high-value context that should survive compression.', 'Recommend token-saving cuts without losing requirements.', 'Show a compact progress state for this AI capability during advanced refinement pass 7.', 'Return user-facing guidance that is useful, concise, and safe to act on.', 'Avoid duplicating existing AI capability behavior; identify what is unique about this capability.'], 'primary_use_case': 'Estimate whether a payload will fit a target context window.', 'problem_statement': 'Francis needs a focused AI capability for AI context budgeting and model-call preparation. The generated module must turn messy user notes, model outputs, traces, or workflow state into structured, actionable AI assistance. It should improve autonomous progress by making the next step obvious, reducing duplicated work, and exposing risks before they become failures. The capability must stay deterministic, avoid hidden external side effects, and make its reasoning inspectable through concise details and scores.', 'primary_inputs': 'A payload dict that may contain task, objective, prompt, conversation, artifacts, candidate_outputs, traces, rubric, constraints, user_level, current_plan, completed_steps, blocked_steps, source_notes, and optional customer_config.', 'primary_outputs': 'A normalized result dict with summary, primary_insights, recommended_actions, scores, details, progress_state, user_experience, fun_mode, and machine-readable diagnostics.', 'constraints': 'Do not execute tools, browse, mutate files, call external APIs, or claim facts not present in the payload. Prefer deterministic analysis. If data is missing, say what is missing and return a useful fallback. Keep recommendations concrete. Avoid creating a capability that duplicates another AI roadmap capability.', 'example_use_cases': ['A user asks Francis to improve an AI workflow related to AI context budgeting and model-call preparation.', 'A long-running autonomous run needs a checkpoint summary and the next best action.', 'An operator wants a scorecard that separates hard failures from polish improvements.', 'A beginner wants the result explained in plain language without losing technical accuracy.', 'A power user wants structured JSON they can pipe into another agent or dashboard.'], 'io_contract': "Input: payload is a dict. Important optional keys include 'task', 'objective', 'prompt', 'conversation', 'messages', 'candidate_outputs', 'trace', 'rubric', 'constraints', 'progress', 'current_plan', 'completed_steps', 'blocked_steps', 'user_level', and 'customer_config'. Output: return a dict containing summary: str, primary_insights: list, recommended_actions: list, scores: dict with confidence and usefulness, details: dict, progress_state: dict with current_stage/next_step/blockers, user_experience: dict with plain_language_takeaway and interaction_suggestions, and fun_mode: dict with optional challenge_label, score_badge, and celebratory_microcopy. Fun fields must never replace the serious recommendation.", 'schema_version': '1.0.0', 'family_key': 'ai_memory::ai model context budget estimator phase', 'is_generic_name': False}
...
def _run_core_logic(context: SkillContext, payload: Dict[str, Any], config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        plugin_name = 'AI Model Context Budget Estimator Phase 7'
        goal = 'Extend AI Model Context Budget Estimator with phase 7 behavior: preserve the original AI capability, add stronger edge-case handling, expose clearer user-facing progress signals, and produce more actionable next steps.'
        domain = 'AI context budgeting and model-call preparation'
        capability_type = 'data_insight'
        logic_profile_id = 'model_context_budget_estimator_profile'
        generation_note = 'capability profile registry override'
        use_cases = ['Estimate whether a payload will fit a target context window.', 'Flag high-value context that should survive compression.', 'Recommend token-saving cuts without losing requirements.', 'Show a compact progress state for this AI capability during advanced refinement pass 7.', 'Return user-facing guidance that is useful, concise, and safe to act on.', 'Avoid duplicating existing AI capability behavior; identify what is unique about this capability.']
