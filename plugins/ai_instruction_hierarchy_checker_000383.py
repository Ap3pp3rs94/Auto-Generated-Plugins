...
Capability: AI Instruction Hierarchy Checker 000383
Slug: ai_instruction_hierarchy_checker_000383
"""
...
_PLUGIN_NAME: str = 'AI Instruction Hierarchy Checker 000383'
_PLUGIN_SLUG: str = 'ai_instruction_hierarchy_checker_000383'
...
_PLUGIN_TAGS = ['content', 'strategy', 'instructions', 'hierarchy', 'safety', 'continuous_backlog', 'use_case_seeded', 'autonomous_factory', 'ai_progress']
...
def _run_core_logic(context: SkillContext, payload: Dict[str, Any], config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        plugin_name = 'AI Instruction Hierarchy Checker 000383'
        ...
        use_cases = ['Apply this capability to the distinct use case: Content Strategy.', 'Find conflicts between instruction layers.', 'Flag unsafe or impossible requirements.', 'Recommend a clarified instruction set.', 'Show a compact progress state for this AI capability during baseline capability.', 'Return user-facing guidance that is useful, concise, and safe to act on.']
        payload_data = payload if isinstance(payload, dict) else {}
        payload_warnings = list(payload_data.get('_payload_warnings', [])) if isinstance(payload_data.get('_payload_warnings'), list) else []
        if not isinstance(payload, dict):
            payload_warnings.append('payload was not a dict; using empty payload')
