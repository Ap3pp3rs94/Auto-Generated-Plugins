...
Capability: AI Response Comparator 000012
Slug: ai_response_comparator_000012
"""
...
_PLUGIN_NAME: str = 'AI Response Comparator 000012'
_PLUGIN_SLUG: str = 'ai_response_comparator_000012'
...
_PLUGIN_USE_CASES = ['Compare candidate responses for completeness and accuracy.', 'Explain why one output is stronger than another.', 'Merge the best parts of multiple responses into a recommendation.', 'Show a compact progress state for this AI capability during baseline capability.', 'Return user-facing guidance that is useful, concise, and safe to act on.', 'Avoid duplicating existing AI capability behavior; identify what is unique about this capability.']
_PLUGIN_RESULT_SCHEMA_VERSION: str = "1.0.0"
_PLUGIN_MANIFEST = {'name': 'AI Response Comparator 000012', 'slug': 'ai_response_comparator_000012', 'goal': 'Compare multiple AI responses and rank them against a task-specific rubric.', 'category': 'ai_evaluation', 'tags': ['ai', 'comparison', 'evaluation', 'rubric', 'autonomous_factory', 'ai_progress'], 'version': '0.1.0', 'capability_type': 'scoring', 'intended_domain': 'AI model comparison and answer selection', 'owner_id': None, 'use_cases': ['Compare candidate responses for completeness and accuracy.', 'Explain why one output is stronger than another.', 'Merge the best parts of multiple responses into a recommendation.', 'Show a compact progress state for this AI capability during baseline capability.', 'Return user-facing guidance that is useful, concise, and safe to act on.', 'Avoid duplicating existing AI capability behavior; identify what is unique about this capability.'], 'schema_version': '1.0.0'}
...
def _run_core_logic(context: SkillContext, payload: Dict[str, Any], config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        plugin_name = 'AI Response Comparator 000012'
        ...
        use_cases = ['Compare candidate responses for completeness and accuracy.', 'Explain why one output is stronger than another.', 'Merge the best parts of multiple responses into a recommendation.', 'Show a compact progress state for this AI capability during baseline capability.', 'Return user-facing guidance that is useful, concise, and safe to act on.', 'Avoid duplicating existing AI capability behavior; identify what is unique about this capability.']
        payload_data = payload if isinstance(payload, dict) else {}
        payload_warnings = list(payload_data.get('_payload_warnings', [])) if isinstance(payload_data.get('_payload_warnings'), list) else []
        if not isinstance(payload, dict):
            payload_warnings.append('payload was not a dict; using empty payload')
