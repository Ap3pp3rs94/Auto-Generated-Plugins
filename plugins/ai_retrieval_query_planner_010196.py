...
Capability: AI Retrieval Query Planner 010196
Slug: ai_retrieval_query_planner_010196
"""
...
_PLUGIN_NAME: str = 'AI Retrieval Query Planner 010196'
_PLUGIN_SLUG: str = 'ai_retrieval_query_planner_010196'
_PLUGIN_CATEGORY: str = 'ai_retrieval'
_PLUGIN_VERSION: str = '0.1.0'
_PLUGIN_GOAL: str = 'Plan focused retrieval queries for AI biotech, lab notebook, assay review, experiment tracking, and research operations workflows for safety controls when evidence is missing or stale.'
_PLUGIN_TAGS = ['biotech', 'lab', 'research', 'safety', 'controls', 'retrieval', 'queries', 'grounding', 'continuous_backlog', 'use_case_seeded', 'autonomous_factory', 'ai_progress']
_PLUGIN_OWNER_ID = 'francis-factory'
_PLUGIN_CAPABILITY_TYPE = 'research_synthesizer'
_PLUGIN_INTENDED_DOMAIN = 'AI biotech, lab notebook, assay review, experiment tracking, and research operations workflows for safety controls retrieval query planning and grounding'
_PLUGIN_USE_CASES = ['Apply this capability to the distinct use case: Biotech Lab Safety Controls.', 'Generate exact and broad discovery queries.', 'Recommend filters, source types, and freshness needs.', 'Separate blocking retrieval from optional enrichment.', 'Show a compact progress state for this AI capability during baseline capability.', 'Return user-facing guidance that is useful, concise, and safe to act on.', 'Avoid duplicating existing AI capability behavior; identify what is unique about this capability.']
_PLUGIN_RESULT_SCHEMA_VERSION: str = "1.0.0"
_PLUGIN_MANIFEST = {'name': 'AI Retrieval Query Planner 010196', 'slug': 'ai_retrieval_query_planner_010196', 'goal': 'Plan focused retrieval queries for AI biotech, lab notebook, assay review, experiment tracking, and research operations workflows for safety controls when evidence is missing or stale.', 'category': 'ai_retrieval', 'tags': ['biotech', 'lab', 'research', 'safety', 'controls', 'retrieval', 'queries', 'grounding', 'continuous_backlog', 'use_case_seeded', 'autonomous_factory', 'ai_progress'], 'version': '0.1.0', 'capability_type': 'research_synthesizer', 'intended_domain': 'AI biotech, lab notebook, assay review, experiment tracking, and research operations workflows for safety controls retrieval query planning and grounding', 'owner_id': 'francis-factory', 'use_cases': ['Apply this capability to the distinct use case: Biotech Lab Safety Controls.', 'Generate exact and broad discovery queries.', 'Recommend filters, source types, and freshness needs.', 'Separate blocking retrieval from optional enrichment.', 'Show a compact progress state for this AI capability during baseline capability.', 'Return user-facing guidance that is useful, concise, and safe to act on.', 'Avoid duplicating existing AI capability behavior; identify what is unique about this capability.'], 'schema_version': '1.0.0'}
...
def _run_core_logic(context: SkillContext, payload: Dict[str, Any], config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        plugin_name = 'AI Retrieval Query Planner 010196'
        goal = 'Plan focused retrieval queries for AI biotech, lab notebook, assay review, experiment tracking, and research operations workflows for safety controls when evidence is missing or stale.'
        domain = 'AI biotech, lab notebook, assay review, experiment tracking, and research operations workflows for safety controls retrieval query planning and grounding'
        ...
        use_cases = ['Apply this capability to the distinct use case: Biotech Lab Safety Controls.', 'Generate exact and broad discovery queries.', 'Recommend filters, source types, and freshness needs.', 'Separate blocking retrieval from optional enrichment.', 'Show a compact progress state for this AI capability during baseline capability.', 'Return user-facing guidance that is useful, concise, and safe to act on.']
        payload_data = payload if isinstance(payload, dict) else {}
        payload_warnings = list(payload_data.get('_payload_warnings', [])) if isinstance(payload_data.get('_payload_warnings'), list) else []
        if not isinstance(payload, dict):
            payload_warnings.append('payload was not a dict; using empty payload')
