...
Capability: AI Prompt Constraint Mapper 000025
Slug: ai_prompt_constraint_mapper_000025
"""
...
_PLUGIN_NAME: str = 'AI Prompt Constraint Mapper 000025'
_PLUGIN_SLUG: str = 'ai_prompt_constraint_mapper_000025'
...
_PLUGIN_USE_CASES = ['Identify hidden or missing constraints in a prompt.', 'Convert vague prompt language into concrete requirements.', 'Recommend prompt rewrites that preserve constraints.', 'Show a compact progress state for this AI capability during baseline capability.', 'Return user-facing guidance that is useful, concise, and safe to act on.', 'Avoid duplicating existing AI capability behavior; identify what is unique about this capability.']
_PLUGIN_RESULT_SCHEMA_VERSION: str = "1.0.0"
_PLUGIN_MANIFEST = {'name': 'AI Prompt Constraint Mapper 000025', 'slug': 'ai_prompt_constraint_mapper_000025', 'goal': 'Map prompt requirements into explicit constraints, missing inputs, and rewrite targets.', 'category': 'ai_prompting', 'tags': ['ai', 'prompting', 'constraints', 'requirements', 'autonomous_factory', 'ai_progress'], 'version': '0.1.0', 'capability_type': 'enrichment', 'intended_domain': 'AI prompt constraints and requirement mapping', 'owner_id': None, 'use_cases': ['Identify hidden or missing constraints in a prompt.', 'Convert vague prompt language into concrete requirements.', 'Recommend prompt rewrites that preserve constraints.', 'Show a compact progress state for this AI capability during baseline capability.', 'Return user-facing guidance that is useful, concise, and safe to act on.', 'Avoid duplicating existing AI capability behavior; identify what is unique about this capability.'], 'schema_version': '1.0.0'}
...
def _run_core_logic(context: SkillContext, payload: Dict[str, Any], config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        plugin_name = 'AI Prompt Constraint Mapper 000025'
        ...
        use_cases = ['Identify hidden or missing constraints in a prompt.', 'Convert vague prompt language into concrete requirements.', 'Recommend prompt rewrites that preserve constraints.', 'Show a compact progress state for this AI capability during baseline capability.', 'Return user-facing guidance that is useful, concise, and safe to act on.', 'Avoid duplicating existing AI capability behavior; identify what is unique about this capability.']
        payload_data = payload if isinstance(payload, dict) else {}
        payload_warnings = list(payload_data.get('_payload_warnings', [])) if isinstance(payload_data.get('_payload_warnings'), list) else []
        if not isinstance(payload, dict):
            payload_warnings.append('payload was not a dict; using empty payload')
