...
Capability: AI Model Fit Triage 000598
Slug: ai_model_fit_triage_000598
"""
...
_PLUGIN_NAME: str = 'AI Model Fit Triage 000598'
_PLUGIN_SLUG: str = 'ai_model_fit_triage_000598'
_PLUGIN_CATEGORY: str = 'ai_agents'
_PLUGIN_VERSION: str = '0.1.0'
_PLUGIN_GOAL: str = 'Triage which model tier or workflow style fits AI legal review and compliance workflows for agentic planning tasks based on risk and complexity.'
_PLUGIN_TAGS = ['legal', 'compliance', 'agentic', 'planning', 'models', 'routing', 'triage', 'continuous_backlog', 'use_case_seeded', 'autonomous_factory', 'ai_progress']
_PLUGIN_OWNER_ID = None
_PLUGIN_CAPABILITY_TYPE = 'scoring'
_PLUGIN_INTENDED_DOMAIN = 'AI legal review and compliance workflows for agentic planning model selection and capability fit'
_PLUGIN_USE_CASES = ['Apply this capability to the distinct use case: Legal Review Agentic Planning.', 'Score whether a task needs fast, cheap, reasoning-heavy, or tool-using flow.', 'Flag tasks that need stronger verification.', 'Recommend the lowest sufficient capability tier.', 'Show a compact progress state for this AI capability during baseline capability.', 'Return user-facing guidance that is useful, concise, and safe to act on.', 'Avoid duplicating existing AI capability behavior; identify what is unique about this capability.']
_PLUGIN_RESULT_SCHEMA_VERSION: str = "1.0.0"
_PLUGIN_MANIFEST = {'name': 'AI Model Fit Triage 000598', 'slug': 'ai_model_fit_triage_000598', 'goal': 'Triage which model tier or workflow style fits AI legal review and compliance workflows for agentic planning tasks based on risk and complexity.', 'category': 'ai_agents', 'tags': ['legal', 'compliance', 'agentic', 'planning', 'models', 'routing', 'triage', 'continuous_backlog', 'use_case_seeded', 'autonomous_factory', 'ai_progress'], 'version': '0.1.0', 'capability_type': 'scoring', 'intended_domain': 'AI legal review and compliance workflows for agentic planning model selection and capability fit', 'owner_id': None, 'use_cases': ['Apply this capability to the distinct use case: Legal Review Agentic Planning.', 'Score whether a task needs fast, cheap, reasoning-heavy, or tool-using flow.', 'Flag tasks that need stronger verification.', 'Recommend the lowest sufficient capability tier.', 'Show a compact progress state for this AI capability during baseline capability.', 'Return user-facing guidance that is useful, concise, and safe to act on.', 'Avoid duplicating existing AI capability behavior; identify what is unique about this capability.'], 'schema_version': '1.0.0'}
...
def _run_core_logic(context: SkillContext, payload: Dict[str, Any], config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        plugin_name = 'AI Model Fit Triage 000598'
        goal = 'Triage which model tier or workflow style fits AI legal review and compliance workflows for agentic planning tasks based on risk and complexity.'
        domain = 'AI legal review and compliance workflows for agentic planning model selection and capability fit'
        capability_type = 'scoring'
        logic_profile_id = 'continuous_model_fit_triage_profile'
        generation_note = 'cadence rename regeneration'
        use_cases = ['Apply this capability to the distinct use case: Legal Review Agentic Planning.', 'Score whether a task needs fast, cheap, reasoning-heavy, or tool-using flow.', 'Flag tasks that need stronger verification.', 'Recommend the lowest sufficient capability tier.', 'Show a compact progress state for this AI capability during baseline capability.', 'Return user-facing guidance that is useful, concise, and safe to act on.']
        payload_data = payload if isinstance(payload, dict) else {}
        payload_warnings = list(payload_data.get('_payload_warnings', [])) if isinstance(payload_data.get('_payload_warnings'), list) else []
        if not isinstance(payload, dict):
            payload_warnings.append('payload was not a dict; using empty payload')
